...
//            return ResultUtil.fail(CodeEnum.PAY_FAIL.val(), CodeEnum.PAY_FAIL.msg());
        ...
        response.sendRedirect("http://localhost:8080/common/paySuccess");
...
//			valueStr = new String(valueStr.getBytes("ISO-8859-1"), "utf-8");
    ...
    public String payMobile(Integer id, HttpServletRequest request) throws AlipayApiException {
        log.info("进入支付接口");
        Orders order = orderService.getById(id);
...
//                + "\"body\":\"" + body + "\","
    ...
    @ApiOperation("功能：支付成功同步跳转")
    @RequestMapping("alipayReturnNoticeMobile")
    public void alipayReturnNoticeMobile(HttpServletRequest request, HttpServletResponse response, Map map) throws Exception {
        log.info("支付成功, 进入同步通知接口...");
        //获取支付宝GET过来反馈信息
        Map<String, String> params = new HashMap<String, String>();
        Map<String, String[]> requestParams = request.getParameterMap();
        ...
        boolean signVerified = AlipaySignature.rsaCheckV1(params, AlipayMobileConfig.alipay_public_key, AlipayMobileConfig.charset, AlipayMobileConfig.sign_type); //调用SDK验证签名
...
//        return ResultUtil.success(map, CodeEnum.PAY_SUCCESS.msg(), CodeEnum.PAY_SUCCESS.val());
     ...
     */
    @ApiOperation("功能：支付成功异步处理")
    @RequestMapping(value = "/alipayNotifyNoticeMoblie")
    public String alipayNotifyNoticeMoblie(HttpServletRequest request, HttpServletRequest response) throws Exception {
